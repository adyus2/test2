...
let create_context func_name =
    ...
    let exit_label = Printf.sprintf ".%s_exit" func_name in
...
let rec gen_expr ctx expr =
    ...
    | FuncCall (name, args) ->
        ...
        (* 总临时空间 = 保存所有临时寄存器(15*4=60) + 额外参数空间 *)
        let temp_space = 60 + n_extra * 4 in
        ...
        let move_args_asm = 
            let rec move_args regs index asm =
                ...
                | reg::rest ->
                    let stack_offset = 60 + (index - 8) * 4 in
        ...
        (* 组合汇编代码 *)
        let asm = arg_asm ^ "\n" ^ stack_adj_asm ^ save_temps_asm ^ "\n" ^ 
                    move_args_asm ^ call_asm ^ restore_temps_asm ^ "\n" ^ 
                    restore_stack_asm ^ "\n" ^ move_result in
...
let gen_function func =
        ...
        let rec gen_save params index asm =
            ...
            | param::rest ->
                ...
                if index < 8 then (
                    ...
                    let (_ctx_temp, reg) = alloc_temp_reg ctx in
    ...
    (* 完整函数代码：序言 + 参数保存 + 函数体 + 退出标签 + 结语 *)
    prologue_asm ^ "\n" ^ save_params_asm ^ body_asm ^ 
    "\n" ^ ctx.exit_label ^ ":" ^ epilogue_asm
